...
module.exports = grammar({
    ...
    rules: {
        ...
        // stmt
        _stmt: ($) => seq(repeat($.attribute), choice(";", $.stmt_if, $.stmt_while, $.stmt_for, $.stmt_block, $.stmt_simple)),
        stmt_simple: ($) => seq(choice($.stmt_return, $.stmt_decl, "continue", "break", $._expr), ";"),
        ...
        expr_unary_pre: ($) => prec.right(seq(field("operation", choice("*", "-", "!", "&")), field("value", $._expr))),
        ...
        _expr_primary: ($) => choice($.expr_tuple, $.expr_identifier, $.expr_sizeof, $.literal),
        ...
        string_raw: ($) => /''(?:.|\n)*?''/,
        string: ($) => seq('"', repeat(choice(alias(token.immediate(/[^\\"\n]+/), $.string_content), $.escape_sequence)), '"'),
        char: ($) => seq("'", repeat(choice(alias(token.immediate(/[^\n']/), $.char_content), $.escape_sequence)), "'"),
        escape_sequence: ($) => choice(prec(2, alias(token(seq("\\", choice(/[\\'"?abfnrtv]/, /\d{1,3}/))), "valid")), prec(1, alias(token(seq("\\", /./)), "invalid"))),
